...
fn run_read_file(input: ReadFileInput) -> Result<String, String> {
    ...
    let output = read_file_in_workspace(&input.path, input.offset, input.limit, &workspace)
...
fn run_glob_search(input: GlobSearchInputValue) -> Result<String, String> {
    let workspace = std::env::current_dir().map_err(|error| error.to_string())?;
    let output = glob_search_in_workspace(&input.pattern, input.path.as_deref(), &workspace)
        .map_err(io_to_string)?;
...
fn run_grep_search(input: GrepSearchInput) -> Result<String, String> {
    let workspace = std::env::current_dir().map_err(|error| error.to_string())?;
    let output = grep_search_in_workspace(&input, &workspace).map_err(io_to_string)?;
        ...
        .filter_map(|message| {
            ...
            let content = message
                ...
                .filter_map(|block| match block {
                    ...
                    // Reasoning is normally dropped on the wire. Set
                    // CLAW_PRESERVE_REASONING to keep prior chain-of-thought in
                    // history (distillation / reasoning-parser round-trips): carry it
...
mod tests {
    ...
    fn agent_tool_subset_mapping_is_expected() {
        ...
        assert!(!verification.contains("power_shell")); // PowerShell removed
  ...
  "nbformat_minor": 5
    ...
    fn bash_tool_reports_success_exit_failure_timeout_and_background() {
        ...
        let timeout = execute_tool("bash", &json!({ "command": "sleep 10", "timeout": 1001 }))
