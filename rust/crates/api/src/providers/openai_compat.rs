...
impl StreamState {
    ...
    fn finish(&mut self) -> Result<Vec<StreamEvent>, ApiError> {
        ...
        // continue after any thinking block because we deliberately
        ...
        let tool_index_offset = if self.dsml_mode {
            dsml_next_index
        } else {
            self.tool_index_offset()
        };
        for state in self.tool_calls.values_mut() {
            if !state.started {
                if let Some(start_event) = state.start_event(tool_index_offset)? {
                    ...
                    let delta = if self.dsml_mode {
                        state.coerced_delta_event(tool_index_offset)
                    } else {
                ...
                if let Some(delta_event) = state.coerced_delta_event(tool_index_offset) {
                    events.push(StreamEvent::ContentBlockDelta(delta_event));
                }
            }
            if state.started && !state.stopped {
...
impl ToolCallState {
    ...
    fn coerced_delta_event(&mut self, offset: u32) -> Option<ContentBlockDeltaEvent> {
...
mod tests {
    use super::{
        ...
        model_emits_dsml_tool_calls, model_requires_reasoning_content_in_history,
        normalize_finish_reason, normalize_response, openai_tool_choice, parse_tool_arguments,
        translate_dsml_tool_calls, OpenAiCompatClient, OpenAiCompatConfig, StreamState,
    };
    use crate::error::ApiError;
    use crate::types::{
        ContentBlockDelta, ContentBlockDeltaEvent, ContentBlockStartEvent, ContentBlockStopEvent,
