...
Столбец про `write_file` относится к собственному инструменту `claw-analog`; у общих встроенных инструментов (`bash`, `edit_file`, …) свои требования — см. раздел [«Инструменты»](#инструменты).
...
| общие встроенные инструменты `claw` | зависит от инструмента | Весь набор из крейта `tools`: `bash`, `edit_file`, `glob_search`, `TodoWrite`, `WebFetch`/`WebSearch`, семейства `Git*`, `Task*`, `Worker*`, MCP и прочие. Имена, совпадающие с собственными инструментами выше, не дублируются — побеждает вариант `claw-analog`. |

Общие инструменты объявляются во **всех** режимах, а проверяются при вызове — так же, как в основном CLI: с включённым Enforcer работает классификация по вводу (читающая команда `bash` разрешена в `read-only`, изменяющая — нет), а с `--no-runtime-enforcer` действует статическое требование инструмента (`bash` → `danger-full-access`). `ToolSearch` не объявляется: здесь ничего не отложено, весь список отдаётся сразу.

Пути в общих инструментах ограничены **рабочим каталогом процесса**: бинарник `claw-analog` переходит (`chdir`) в `-w` перед циклом агента, и вызов отклоняется, если каталог не совпадает с корнем workspace. Собственные инструменты из таблицы выше остаются самым дешёвым способом читать и искать по репозиторию: относительные пути и отдельные лимиты (`--max-read-bytes`, `--grep-max-lines`, `--glob-max-paths`).
